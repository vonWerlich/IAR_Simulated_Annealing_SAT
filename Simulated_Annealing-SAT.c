...
#define N 10000 //numero de iterações total 
#define T0 50
#define Tn 0.001
...
typedef struct {
    int indices[MAX_ALTERACOES]; // no máximo dois índices modificados
    int count;      // 1 ou 2
} Alteracao;
...
//estratégia média -> vários vizinhos locais
Alteracao gerarVizinho2(bool *vizinho, const bool *solucao) {
    ...
    if (metodo == 0) {  // troca dois bits //swap -> ineficiente 
        int i = rand() % num_vars;
        int j = rand() % num_vars;
...
bool* gerarVizinho3(bool *solucao) {
    bool* vizinho = criarVetor(num_vars);
    int i;
	for (i = 0; i < num_vars; i++) {
        vizinho[i] = solucao[i];
    }
    
    int metodo = rand() % 4; // Escolhe um dos três métodos

    if (metodo == 0) {  // Troca de duas posições
        int i = rand() % num_vars;
        int j = rand() % num_vars;
        while (i == j) j = rand() % num_vars; 
    ...
    } else if (metodo == 1) {  // Alteração de um único bit
        int i = rand() % num_vars;
        vizinho[i] = !vizinho[i];

    } else if (metodo == 2) {  // Inverter múltiplos bits
        int num_flips = 1 + rand() % 3; // Entre 1 e 3 bits
        for (int k = 0; k < num_flips; k++) {
            int i = rand() % num_vars;
            vizinho[i] = !vizinho[i];
        }
	
	//randomiza tudo de novo
    } else if (metodo == 3) {
    	for (int i = 0; i < num_vars; i++) {
        	vizinho[i] = rand() % 2;
    	}
...
// Aceitação com Probabilidade em Cada Vizinho
	...
	while (T > Tn) {
		...
		while (iterations < SA_MAX) {
			//printf("%d", iterations);
			iterations++;
			Alteracao alt = gerarVizinho1(vizinho, solution);//gerar vizinho
		...
		T = coolingSchedule1(i, T); // nova temperatura
